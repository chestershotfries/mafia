...
def record_game(body):
    ...
    current_game_id = (
        history_data[1][0] if len(history_data) > 1 and history_data[1][0] else None
    )
    ...
    for a in sorted(assignments, key=lambda x: x["position"]):
        ...
        # Vigilante) so power roles show in the last-recorded-game and
        # match-history views — including a power seat filled by a ghost
        # (e.g. a ghost vigilante). Team split and win/loss still key off
        # Mafia-vs-not, so ratings are unaffected.
        alignment = POSITION_ROLES.get(a["position"], a["role"])
    ...
    for a in rated:
        ...
        if cr:
            ...
            ws_ratings.update(
                f"A{next_row}:C{next_row}", [[name, nr["mu"], nr["sigma"]]]
            )
...
def undo_last_game():
    ...
    players_to_delete = [
        p["name"] for p in players_to_restore if p["name"] not in remaining_players
    ]
...
def get_stats():
    ...
    return {
        ...
        "game_summary": {
            ...
            "town_wins": town_wins,
            "mafia_win_pct": round(100 * mafia_wins / total_games, 1)
            if total_games
            else 0,
            "town_win_pct": round(100 * town_wins / total_games, 1)
            if total_games
            else 0,
        },
    }
...
def _check_password(body):
    if body.get("password") != GAME_PASSWORD:
